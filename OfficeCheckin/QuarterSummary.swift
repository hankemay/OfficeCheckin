...
struct QuarterSummary: Identifiable {
    ...
    static func all(from checkins: [CheckIn], through date: Date = .now) -> [QuarterSummary] {
        let calendar = Calendar.current
        let workingDayCheckins = checkins.filter { !calendar.isDateInWeekend($0.checkedInAt) }
        guard let first = workingDayCheckins.map(\.checkedInAt).min(),
        ...
        while cursor <= currentQuarter.start, let interval = calendar.dateInterval(of: .quarter, for: cursor) {
            let count = workingDayCheckins.filter { interval.contains($0.checkedInAt) }.count
